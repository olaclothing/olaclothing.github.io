let STRATEGIES,formatMessage;_34e‍.x([["createExtendRoutesHook",()=>createExtendRoutesHook],["buildHook",()=>buildHook]]);_34e‍.w("../helpers/constants",[["STRATEGIES",["STRATEGIES"],function(v){STRATEGIES=v}]]);_34e‍.w("../templates/utils-common",[["formatMessage",["formatMessage"],function(v){formatMessage=v}]]);
 ...
 */
  ...
  if (options.strategy === STRATEGIES.NO_PREFIX && options.differentDomains) {
    ...
    _34e‍.g.console.warn(formatMessage('The `differentDomains` option and `no_prefix` strategy are not compatible. Change strategy or disable `differentDomains` option.'))
  }

  if ('forwardedHost' in options) {
    // eslint-disable-next-line no-console
    _34e‍.g.console.warn(formatMessage('The `forwardedHost` option is deprecated. You can safely remove it. See: https://github.com/nuxt-community/i18n-module/pull/630.'))
  }
  ...
  if (options.vueI18nLoader) {
    this.extendBuild(config => {
      if (!config.module) {
        _34e‍.g.console.warn(formatMessage('Failed to register the vue-i18n-loader.'))
        return
      }
